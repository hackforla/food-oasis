...
const SelectedStakeholderDisplay = (props) => {
    ...
    return <div className={classes.stakeholderHolder}>
        <div className={classes.topInfoHolder}>
            <div className={classes.imgHolder}>
                <img src={selectedStakeholder.categories[0].id === 1 ? pantryIcon : mealIcon}
            ...
            <div className={classes.infoHolder}>
                ...
                <em style={{ color: selectedStakeholder.categories[0].id === 1 ? "#336699" : "#CC3333" }}>
                    {selectedStakeholder.categories[0].name}
                </em>
            </div>
            <div className={classes.checkHolder}>
                {selectedStakeholder.distance >= 10 ?
                    selectedStakeholder.distance.toString().substring(0, 3).padEnd(4, "0") :
                    selectedStakeholder.distance.toString().substring(0, 3)} mi
        </div>
        </div>
        ...
        <div className={classes.hoursContainer}>
            {selectedStakeholder.hours.map(hour =>
                <div className={classes.singleHourContainer}>
                    ...
                    <span>{standardTime(hour.open)}-{
                        standardTime(hour.close)}</span>
                </div>)}
        </div>
