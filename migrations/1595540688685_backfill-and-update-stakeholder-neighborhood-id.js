...
exports.up = (pgm) => {
  // Backfill neighborhood id for current stakeholders
  let updateSql = `UPDATE stakeholder SET neighborhood_id=(SELECT id FROM neighborhood
    WHERE ST_Contains(
    geometry,
    ST_Point(longitude, latitude)) LIMIT 1
  );`;
  pgm.sql(updateSql);
  // Update any records in stakeholder_best that didn't get auto-populated from the
  // previous stakeholder table update
  updateSql = `UPDATE stakeholder_best SET neighborhood_id=(SELECT id FROM neighborhood
    WHERE ST_Contains(
    geometry,
    ST_Point(longitude, latitude)) LIMIT 1) WHERE neighborhood_id is null
  ;`;
  pgm.sql(updateSql);

  // Modify our stakeholder table trigger to correctly set the neighborhood ID when a stakeholder is created
  // or updated.
  const newTriggerDef = `
...
$BODY$;`;
  ...
  END;
  $$ LANGUAGE plpgsql;`;
  pgm.sql(updateProcSql);
};

exports.down = (pgm) => {
  const originalTriggerDef = `
  CREATE OR REPLACE FUNCTION ${TRIGGER_NAME}()
RETURNS trigger
LANGUAGE 'plpgsql'
COST 100
...
$BODY$;`;
  ...
  pgm.createTrigger(SOURCE_TABLE, TRIGGER_NAME, {
    ...
    functionParams: [],
    level: "ROW",
    definition: originalTriggerDef,
  });

  const origCreateProcSql = `CREATE OR REPLACE PROCEDURE public.create_stakeholder(
    s_id INOUT integer,
    s_name character varying,
    s_address_1 character varying,
    s_address_2 character varying,
  ...
  pgm.sql(origCreateProcSql);

  const origUpdateProcSql = `CREATE OR REPLACE PROCEDURE update_stakeholder(
    s_name VARCHAR, s_address_1 VARCHAR, s_address_2 VARCHAR, s_city VARCHAR, s_state VARCHAR, s_zip VARCHAR,
    s_phone VARCHAR, s_latitude NUMERIC, s_longitude NUMERIC, s_website VARCHAR, s_inactive BOOLEAN,
    s_notes VARCHAR, s_requirements VARCHAR, s_admin_notes VARCHAR, s_parent_organization VARCHAR,
    s_physical_access VARCHAR, s_email VARCHAR, s_items VARCHAR, s_services VARCHAR, s_facebook VARCHAR,
